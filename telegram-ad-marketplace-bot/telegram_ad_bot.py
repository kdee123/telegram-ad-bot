...
class AdMarketplaceBot:
    ...
    def handle_message(self, message: dict[str, Any]) -> None:
        ...
        if not text.startswith("/"):
            if chat_id:
                self.api.send_message(chat_id, "Send /help to see the marketplace commands.")
            return
        ...
        if command in {"start", "help"}:
            ...
            self.api.send_message(chat_id, "Unknown command. Send /help for the command list.")
        ...
        self.api.send_message(
            ...
            "\n".join(
                [
                    ...
                    "/my_channels",
                    "/set_stars_price @channel 250",
                    "/set_ton_wallet @channel wallet_address",
                    "/set_crypto_wallet @channel wallet_or_payment_instructions",
                    "/earnings",
                    "/request_payout XTR payout_account_or_notes",
                    "/disable @channel",
                    "/enable @channel",
                    "",
                    "Advertisers:",
                    "/list",
                    "/list category",
                    "/request @channel stars Your ad text here",
                    "/request @channel ton Your ad text here",
                    "/request @channel crypto Your ad text here",
                    "/confirm_crypto request_id transaction_id",
                    "/requests",
                    "",
                    "Stars earnings are tracked for owner payouts. TON and crypto go directly to owner wallets.",
                ]
            ),
        )
    ...
    def cmd_register_channel(self, chat_id: int, user: dict[str, Any], args: str) -> None:
        ...
        if len(parts) < 3:
            self.api.send_message(
                chat_id,
                "Usage: /register_channel @channel price category audience\n"
                "Example: /register_channel @dailytech $50 tech 25000_subscribers",
            )
            return
        ...
        try:
            ...
            if member.get("status") not in ADMIN_STATUSES:
                self.api.send_message(chat_id, f"You must be an admin of {handle} to register it.")
                return
            ...
            if bot_member.get("status") not in ADMIN_STATUSES or not can_post:
                self.api.send_message(
                    chat_id,
                    f"Add this bot as an admin in {handle} with permission to post messages, then try again.",
                )
                return
        except TelegramAPIError as exc:
            self.api.send_message(
                chat_id,
                "I could not verify that channel. Make sure it is public, the username is correct, "
                "and this bot has been added as an admin.\n\n"
                f"Telegram response: {exc}",
            )
            return
        ...
        self.api.send_message(
            chat_id,
            f"Registered {title} ({handle}). Advertisers can now request placements with /request {handle}.",
        )

    def cmd_my_channels(self, chat_id: int, user: dict[str, Any]) -> None:
        rows = self.store.owner_channels(int(user["id"]))
        if not rows:
            self.api.send_message(chat_id, "You have not registered any channels yet.")
            return
        ...
        for row in rows:
            ...
            lines.append(
                f"{row['handle']} - {row['title']} - {row['price']} - {stars} - {ton} - {crypto} - {row['category']} - {status}"
            )
    ...
    def cmd_request(self, chat_id: int, user: dict[str, Any], args: str) -> None:
        ...
        if payment_method == "stars" and not channel["stars_price"]:
            ...
            return
        if payment_method == "ton" and not channel["ton_wallet"]:
            self.api.send_message(chat_id, f"{handle} has not enabled TON payments.")
            return
        if payment_method == "crypto" and not channel["crypto_wallet"]:
            self.api.send_message(chat_id, f"{handle} has not enabled crypto payments.")
            return
    ...
    def payment_summary(self, channel: sqlite3.Row, payment_method: str) -> str:
        ...
        if payment_method == "ton":
            return "TON payment to owner wallet"
        if payment_method == "crypto":
            return "crypto payment to owner wallet"
    ...
    def cmd_confirm_crypto(self, chat_id: int, user: dict[str, Any], args: str) -> None:
        ...
        self.api.send_message(chat_id, "Payment reference submitted. The owner will verify it.")
    ...
    def handle_callback(self, callback: dict[str, Any]) -> None:
        ...
        if not request:
            ...
            return
        if int(request["owner_user_id"]) != user_id:
            self.api.answer_callback_query(callback_id, "Only the channel owner can decide this request.")
            return
        ...
        if request["payment_method"] in {"ton", "crypto"} and request["payment_status"] != "paid":
            wallet = request["ton_wallet"] if request["payment_method"] == "ton" else request["crypto_wallet"]
            self.store.set_request_status(request_id, "approved_pending_payment")
    ...
    def credit_owner_after_post(self, request: sqlite3.Row) -> None:
        if request["payment_method"] != "stars" or request["payment_status"] != "paid":
            return

        gross_amount = int(request["stars_amount"] or 0)
        if gross_amount <= 0:
            return
        ...
        if credited:
            self.api.send_message(
                request["owner_chat_id"],
                f"Earned {owner_amount} XTR from request #{request['id']} "
                f"after {platform_fee} XTR platform fee.",
            )
